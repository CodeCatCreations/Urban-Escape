...
https.get('https://miljodataportalen.stockholm.se/api/esbo-2017-ekologisktsarskiltbetydelsefullaomraden', options, (response) => {
  let data = '';
    ...
    parseString(data, (err, result) => {
      ...
      for (const geom of geoms) {
        ...
        if (!geom.includes("EMPTY") && subGeom.indexOf('((') === -1 && subGeom.indexOf(')') === -1) {
          ...
          polygons.push(`INSERT INTO esbopolygons (id, name) VALUES (${currentId}, 'Esbopolygon ${currentId}');`);
          for (let i = 0; i < coords.length; i++) {
            ...
            coordinates.push(`INSERT INTO esbocoordinates (esbopolygon_id, latitude, longitude) VALUES (${currentId}, ${lat}, ${lng});`);
          }
          currentId++;
        }
      }
      ...
      fs.writeFile('esbo_single_polygons.sql', polygons.join('\n'), (err) => {
        if (err) throw err;
        console.log('Polygons written to polygons.sql');
      });

      fs.writeFile('esbo_single_coordinates.sql', coordinates.join('\n'), (err) => {
        if (err) throw err;
