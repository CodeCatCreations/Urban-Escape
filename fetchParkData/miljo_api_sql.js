...
const request = https.request(options, (response) => {
    ...
    response.on('end', () => {
        ...
        const outputFile = 'polygon_buller_api.sql';

        // Open output file for writing
        const stream = fs.createWriteStream(outputFile);
        stream.once('open', () => {
            // Write SQL statements for each polygon
            // Write SQL statements for each polygon
            // Write SQL statements for each polygon
            polygons.forEach((polygon, index) => {
                ...
                formattedCoordinates.forEach((coordinate, i) => {
                    stream.write(`INSERT INTO polygons (id, latitude, longitude) VALUES (${index + 1}, ${coordinate.lat}, ${coordinate.lng});\n`);
                });
            });
...
https.get('https://miljodataportalen.stockholm.se/api/buller-2022-vagtagflygleq24h', options, (response) => {
  ...
  response.on('end', () => {
    fs.writeFile('Buller_API.json', data, (err) => {
      if (err) throw err;
      console.log('File written to Buller_API.json');
    });
  });
