...
https.get('https://miljodataportalen.stockholm.se/api/buller-2022-vagtagleq24h?isov1=>70', options, (response) => {
  let data = '';
  ...
  response.on('end', () => {
    parseString(data, (err, result) => {
      ...
      fs.writeFile('above70dbPolygons.sql', polygons.join('\n'), (err) => {
        if (err) throw err;
        console.log('Polygons written to above70dbPolygons.sql');
      });

      fs.writeFile('above70dbCoordinates.sql', coordinates.join('\n'), (err) => {
        if (err) throw err;
        console.log('Coordinates written to above70dbCoordinates.sql');
      });
    });
  });
}).on('error', (err) => {
