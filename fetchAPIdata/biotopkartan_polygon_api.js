...
https.get('https://miljodataportalen.stockholm.se/api/biotopkartan-2019-biotoper', options, (response) => {
  let data = '';
    ...
    parseString(data, (err, result) => {
      ...
      for (const geom of geoms) {
        ...
        if (!geom.includes("EMPTY") && subGeom.indexOf('((') !== -1 || subGeom.indexOf(')') !== -1) {
          ...
          polygons.push(`INSERT INTO biotoppolygons (id, name) VALUES (${currentId}, 'Biotoppolygon ${currentId}');`);
          for (let i = 0; i < coordsSets.length; i++) {
            ...
            for (let j = 0; j < coords.length; j++) {
              ...
              coordinates.push(`INSERT INTO biotopcoordinates (biotoppolygon_id, latitude, longitude) VALUES (${currentId}, ${lat}, ${lng});`);
            }
          }
          currentId++;
        }
      ...
      fs.writeFile('biotoppolygon.sql', polygons.join('\n'), (err) => {
        if (err) throw err;
        console.log('Polygons written to polygons.sql');
      });

      fs.writeFile('biotopcoordinates.sql', coordinates.join('\n'), (err) => {
        if (err) throw err;
